...
void* openFile(void *arg){
	...
	if(strncmp(arg,"reading",7) == 0){	
		...
		*fd = open(getFile, O_WRONLY); // open file in write mode or create the file if it is not present
	}
	else if(strncmp(arg, "creat|writing", 13) == 0){
		*fd = open(getFile, O_CREAT | O_WRONLY); // open file in write mode or create the file if it is not present
	}
